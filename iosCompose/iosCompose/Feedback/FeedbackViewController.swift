...
override func viewDidLoad() {
    ...
    ratingGoodButton.setBackgroundImage(tintableImageGood,for: UIControl.State.normal)
        
        let tintableImageOk = ratingOkButton.currentBackgroundImage?.withRenderingMode(.alwaysTemplate)
    ratingOkButton.setBackgroundImage(tintableImageOk,for: UIControl.State.normal)
        
        let tintableImageBad = ratingBadButton.currentBackgroundImage?.withRenderingMode(.alwaysTemplate)
    ratingBadButton.setBackgroundImage(tintableImageBad,for: UIControl.State.normal)
    ...
    commentTextView.textContainerInset = UIEdgeInsets(top: 16, left: 8, bottom: 0, right: 8)
